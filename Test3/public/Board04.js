...
function scoring(){
	...
	document.getElementById("title").innerHTML = "Spectators: " + speC;
}
...
socket.on('leftPlayerUpdate', function(data){
	player[0].init(data.px, data.py, data.vis, data.mx, data.my, data.dir);
});

socket.on('rightPlayerUpdate', function(data){
	player[1].init(data.px, data.py, data.vis, data.mx, data.my, data.dir);
});
